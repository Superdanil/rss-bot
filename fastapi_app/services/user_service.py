...
from services.service_depends import IUserRepository, IUserSourceAssociationRepository
...
class UserService:
    ...
    async def add_association(self, user_id: UUID, source_id: UUID, session) -> None:
        ...
        await self._user_source_assoc_repository.create(user_id, source_id, session)
    ...
    async def get_user(self, telegram_id: int, session: AsyncSession) -> UserReadDTO:
        ...
        return await self._repository.create(telegram_id, session)
